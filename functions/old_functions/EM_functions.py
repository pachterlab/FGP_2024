...
def loss(theta, x_weighted, marginal_weight, t, tau, topo, penalty):
    loss = - logL    
...
def neglogL_bg(theta_bg,theta_a,x,Q,t,tau,topo):
    theta = np.append(theta_a, theta_bg)
    loss = neglogL(theta,x,Q,t,tau,topo)
    return loss

def neglogL_a(theta_a,theta_bg,x,Q,t,tau,topo):
    theta = np.append(theta_a, theta_bg)
    loss = neglogL(theta,x,Q,t,tau,topo)
    return loss

def update_theta_j(theta0, x, Q, t, tau, topo, alternative, bnd=1000, bnd_beta=100, miter = 1000, alteriter = 10):
    ...
    if alternative:
        ...
        for i in range(alteriter):
            ## minimize wrt a
            res1 = minimize(neglogL_a, new_theta[:-2], args=(new_theta[-2:],x,Q,t,tau,topo), bounds = bounds[:-2], options={'maxiter': int(miter/alteriter/2),'disp': False}) 
            new_theta[:-2]=res1.x
            ## minimize wrt beta and gamma
            res2 = minimize(neglogL_bg, new_theta[-2:], args=(new_theta[:-2],x,Q,t,tau,topo), bounds = bounds[-2:], options={'maxiter': int(miter/alteriter/2),'disp': False}) 
            new_theta[-2:]=res2.x
    else:
        res = minimize(neglogL, theta0, args=(x_weighted,marginal_weight,t,tau,topo), bounds=bounds, options={'maxiter': miter,'disp': True}) 
        new_theta = res.x
    return new_theta
